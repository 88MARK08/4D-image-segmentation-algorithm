...
double _l2norm_mcf(double *dataArray3DPtr1, double *dataArray3DPtr2)
{
	...
	/*Computation of norm*/
	int i, j, k, l;
#pragma omp parallel for private(i, j, k) reduction(+:sumPower)
	for (l = 0; l < lMax; l++)
		for (k = 0; k < kMax; k++)
			for (j = 0; j < jMax; j++)
				for (i = 0; i < iMax; i++)
...
#pragma omp parallel for private(i, j, k) reduction(+:mean_square_residue)
		for (l = 1; l < lMax - 1; l++)
			for (k = 1; k < kMax - 1; k++)
				for (j = 1; j < jMax - 1; j++)
					for (i = 1; i < iMax - 1; i++)
...
bool _set4DBoundaryToExactValues_mcf(double t)
{
	...
	// X Direction
#pragma omp parallel for private(j, k, x, y, z, w)
	for (l = 0; l < lMax; l++)
		for (k = 0; k < kMax; k++)
			for (j = 0; j < jMax; j++)
			{
...
#pragma omp parallel for private(j, k, x, y, z, w)
	...
	// Y Direction
#pragma omp parallel for private(i, k, x, y, z, w)
	for (l = 0; l < lMax; l++)
		for (k = 0; k < kMax; k++)
			for (i = 0; i < iMax; i++)
			{
...
#pragma omp parallel for private(i, k, x, y, z, w)
	...
	// Z Direction
#pragma omp parallel for private(i, j, x, y, z, w)
	for (l = 0; l < lMax; l++)
		for (j = 0; j < jMax; j++)
			for (i = 0; i < iMax; i++)
			{
...
#pragma omp parallel for private(i, j, k, x, y, z, w)
	for (l = 0; l < lMax; l++)
		for (k = 0; k < kMax; k++)
			for (j = 0; j < jMax; j++)
				for (i = 0; i < iMax; i++)
